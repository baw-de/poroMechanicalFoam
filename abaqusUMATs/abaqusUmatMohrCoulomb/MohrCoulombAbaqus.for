...
!   CMNAME               -  no. of direct stress/strain components
...
!----- For summing up regions ---
	!STATEV(region+1) = real(STATEV(region+1) + 1,8)
!--------------------------------
...
!     ouplP (integer,sc)  Only used in plane situations in which it is similar to
...
!--------------------------------------------------------------------
      ...
      integer(4) i, j
      integer(4), dimension(3), parameter :: Hj1 = (/1, 3, 2/)
      integer(4), dimension(3), parameter :: Hj2 = (/2, 1, 3/)
...
!--------------------------------------------------------------------------
	...
	if (nsigma == 3) then ! Sigma is the principal stresses
		S = Sigma - I1/3 ! Deviator stress vector
		J2 = 0.5_8*sum(S**2)
		J3 = S(1)*S(2)*S(3)
    
	elseif (nsigma == 4) then ! Plane situation, including axisymmetry
		S(1:3) = Sigma(1:3) - I1/3 ! Deviator stress vector
		S(4) = Sigma(4)
		J2 = 0.5_8 * sum(S(1:3)**2) + S(4)**2
    
		J3 = (S(1)**3 + S(2)**3 + S(3)**3 +3.0_8*S(4)**2 *(S(1)+S(2))) / 3
