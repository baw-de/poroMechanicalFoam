...
namespace Foam
{
    ...
    autoPtr<effectiveStressModel> effectiveStressModel::New(
      const dictionary &effectiveStressModelDict,
      const word &name,
      const fvMesh &mesh)
    {

        Info << "Selecting effective stress model: "
             << name << endl;
#if (OPENFOAM >= 2112)
...
#endif
        return autoPtr<effectiveStressModel>(ctorPtr(effectiveStressModelDict, name, mesh));
    }
