...
void Foam::poroSubMeshes::makeSubMeshes() const
{
    if (subMeshes_.valid())
    {
        FatalErrorIn("void Foam::poroSubMeshes::makeSubMeshes() const")
    ...
    subMeshes_.setSize(cellZoneNames_.size());
...
/*
void Foam::poroSubMeshes::makeSubMeshVolToPoint() const
{
...
void Foam::poroSubMeshes::checkCellZones() const
{
    if (cellZoneNames_.size() == 1)
    {
    ...
    forAll(cellZoneNames_, matI)
    {
        ...
        if (!cellZoneID.active())
        {
            FatalErrorIn("void Foam::poroSubMeshes::checkCellZones()")
...
/*
void Foam::poroSubMeshes::calcSubMeshFlux() const
{
    if (!subMeshFlux_.empty())
    {
        FatalErrorIn("void Foam::poroSubMeshes::calcSubMeshFlux() const")
    ...
    subMeshFlux_.setSize(cellZoneNames_.size());
...
void Foam::poroSubMeshes::calcSubMeshFluxf() const
{
    if (!subMeshFluxf_.empty())
    {
        FatalErrorIn("void Foam::poroSubMeshes::calcSubMeshFluxf() const")
            << "pointer list already set" << abort(FatalError);
    }
    ...
    subMeshFluxf_.setSize(cellZoneNames_.size());
...
void Foam::poroSubMeshes::calcSubMeshD() const
{
    if (!subMeshD_.empty())
    {
        FatalErrorIn("void Foam::poroSubMeshes::calcSubMeshD() const")
            << "pointer list already set" << abort(FatalError);
    }
...
void Foam::poroSubMeshes::calcSubMeshGradD() const
{
    if (!subMeshGradD_.empty())
    {
        FatalErrorIn("void Foam::poroSubMeshes::calcSubMeshGradD() const")
            << "pointer list already set" << abort(FatalError);
    }
...
void Foam::poroSubMeshes::calcSubMeshGradDf() const
{
    if (!subMeshGradDf_.empty())
    {
        FatalErrorIn("void Foam::poroSubMeshes::calcSubMeshGradDf() const")
            << "pointer list already set" << abort(FatalError);
    }
...
void Foam::poroSubMeshes::calcSubMeshPointD() const
{
    if (!subMeshPointD_.empty())
    {
        FatalErrorIn("void Foam::poroSubMeshes::calcSubMeshPointD() const")
            << "pointer list already set" << abort(FatalError);
    }
...
*/
/*
void Foam::poroSubMeshes::calcInterfaceShadowIDs() const
{
    if
    (
    ...
    labelList baseMeshShadSubMeshID = labelList(baseMesh().nInternalFaces(), -1);
    labelList baseMeshShadPatchID = labelList(baseMesh().nInternalFaces(), -1);
    labelList baseMeshShadFaceID = labelList(baseMesh().nInternalFaces(), -1);
    ...
    forAll(cellZoneNames_, matI)
    {
        ...
        forAll(subMesh.boundaryMesh(), patchI)
        {
            if (patchMap[patchI] == -1)
            {
                ...
                labelList& shadSubMeshID = interfaceShadowSubMeshID_[matI];
                labelList& shadPatchID = interfaceShadowPatchID_[matI];
                labelList& shadFaceID = interfaceShadowFaceID_[matI];
                ...
                forAll(shadSubMeshID, faceI)
                {
                    ...
                    if (baseMesh().isInternalFace(baseFaceID))
                    {
                        ...
                        if (baseMeshShadSubMeshID[baseFaceID] == -1)
                        {
                            ...
                            baseMeshShadSubMeshID[baseFaceID] = matI;
...
Foam::poroSubMeshes::interfaceShadowSubMeshID() const
{
...
const Foam::PtrList<Foam::labelList>&
Foam::poroSubMeshes::interfaceShadowPatchID() const
{
...
const Foam::PtrList<Foam::labelList>&
Foam::poroSubMeshes::interfaceShadowFaceID() const
{
...
void Foam::poroSubMeshes::makeInterfaceBaseFaces() const
{
...
const Foam::labelList& Foam::poroSubMeshes::interfaceBaseFaces() const
{
    if (!interfaceBaseFacesPtr_)
    {
...
void Foam::poroSubMeshes::makePointNumOfMaterials() const
{
...
const Foam::labelList& Foam::poroSubMeshes::pointNumOfMaterials() const
{
    if (!pointNumOfMaterialsPtr_)
    {
...
void Foam::poroSubMeshes::makeIsolatedInterfacePoints() const
{
...
const Foam::labelList& Foam::poroSubMeshes::isolatedInterfacePoints() const
{
    if (!isolatedInterfacePointsPtr_)
    {
...
Foam::poroSubMeshes::interfaceShadowFlux() const
{
    if (interfaceShadowFlux_.empty())
    {
        makeInterfaceShadowFlux();
    }

    return interfaceShadowFlux_;
}


void Foam::poroSubMeshes::makeInterfaceShadowFlux() const
{
    if (!interfaceShadowFlux_.empty())
    {
        FatalErrorIn
    ...
    interfaceShadowFlux_.setSize(subMeshes().size());
    ...
    forAll(subMeshes(), subMeshI)
    {
        ...
        if (patchID == -1)
        {
            ...
            interfaceShadowFlux_.set(subMeshI, new symmTensorField(0));
        }
        else
        {
            interfaceShadowFlux_.set
            (
                subMeshI,
                new symmTensorField
                (
...
void Foam::poroSubMeshes::updateInterfaceShadowFlux
(
...
)
{
    ...
    volSymmTensorField baseFluxForSyncing
    (
        IOobject
        (
            "baseFluxForSyncing",
    ...
    forAll(subMeshes(), subMeshI)
    {
        ...
        symmTensorField& resultFlux = interfaceShadowFlux_[subMeshI];
        ...
        forAll(resultFlux, faceI)
        {
            ...
            else
            {
                ...
                if (useVolFieldFlux)
                {
                    baseFluxForSyncing.internalField()
                        [
    ...
    // patch
    baseFluxForSyncing.correctBoundaryConditions();
    ...
    forAll(subMeshes(), subMeshI)
    {
        ...
        forAll(resultFlux, faceI)
        {
            ...
            if (interfaceShadowSubMeshID[faceI] == -1)
            {
                ...
                resultFlux[faceI] =
                    baseFluxForSyncing.boundaryField()
                    [
                        basePatchID
                    ][baseLocalFaceID];
            }
...
bool Foam::poroSubMeshes::biMaterialInterfaceActive() const
{
...
void Foam::poroSubMeshes::calcBiMaterialInterfaceActive() const
{
    if (biMaterialInterfaceActivePtr_)
    {
        FatalErrorIn
...
void Foam::poroSubMeshes::clearOut()
{
...
Foam::poroSubMeshes::poroSubMeshes
(
...
const Foam::fvMesh& Foam::poroSubMeshes::baseMesh() const
{
    return baseMesh_;
}
...
Foam::poroSubMeshes::subMeshes() const
{
    if (subMeshes_.empty())
    {
        makeSubMeshes();
...
Foam::PtrList<Foam::newFvMeshSubset>& Foam::poroSubMeshes::subMeshes()
{
    if (subMeshes_.empty())
    {
        makeSubMeshes();
...
Foam::poroSubMeshes::subMeshVolToPoint() const
{
    if (subMeshVolToPoint_.empty())
    {
...
*/
/*
Foam::PtrList<Foam::volSymmTensorField>& Foam::poroSubMeshes::subMeshFlux()
{
    if (subMeshFlux_.empty())
    {
        calcSubMeshFlux();
...
Foam::poroSubMeshes::subMeshFlux() const
{
    if (subMeshFlux_.empty())
    {
        calcSubMeshFlux();
...
Foam::poroSubMeshes::subMeshFluxf()
{
    if (subMeshFluxf_.empty())
    {
        calcSubMeshFluxf();
    ...
    return subMeshFluxf_;
}


const Foam::PtrList<Foam::surfaceSymmTensorField>&
Foam::poroSubMeshes::subMeshFluxf() const
{
    if (subMeshFluxf_.empty())
    {
        calcSubMeshFluxf();
...
Foam::PtrList<Foam::volVectorField>& Foam::poroSubMeshes::subMeshD()
{
    if (subMeshD_.empty())
    {
        calcSubMeshD();
...
Foam::poroSubMeshes::subMeshD() const
{
    if (subMeshD_.empty())
    {
        calcSubMeshD();
...
Foam::PtrList<Foam::volTensorField>& Foam::poroSubMeshes::subMeshGradD()
{
    if (subMeshGradD_.empty())
    {
        calcSubMeshGradD();
...
Foam::poroSubMeshes::subMeshGradD() const
{
    if (subMeshGradD_.empty())
    {
        calcSubMeshGradD();
...
Foam::PtrList<Foam::surfaceTensorField>& Foam::poroSubMeshes::subMeshGradDf()
{
    if (subMeshGradDf_.empty())
    {
        calcSubMeshGradDf();
...
Foam::poroSubMeshes::subMeshGradDf() const
{
    if (subMeshGradDf_.empty())
    {
        calcSubMeshGradDf();
...
Foam::PtrList<Foam::pointScalarField>& Foam::poroSubMeshes::subMeshPointp()
{
    if (subMeshPointD_.empty())
    {
        calcSubMeshPointp();
...
const Foam::PtrList<Foam::pointScalarField>&
Foam::poroSubMeshes::subMeshPointp() const
{
    if (subMeshPointD_.empty())
    {
        calcSubMeshPointp();
...
*/
/*
void Foam::poroSubMeshes::interpolateDtoSubMeshD
(
...
)
{
    ...
    // Update the interface shadow flux fields
    // This can require parallel communication
    updateInterfaceShadowFlux(useVolFieldFlux);
    ...
    forAll(subMeshes, matI)
    {
        ...
        forAll(subMeshD.boundaryField(), patchI)
        {
            if (patchMap[patchI] == -1)
            {
                ...
                const symmTensorField* fluxPatchPtr = NULL;
                if (useVolFieldFlux)
                {
                    fluxPatchPtr =
                        &(subMeshFlux()[matI].boundaryField()[patchI]);
                }
                else
                {
                    fluxPatchPtr =
                        &(subMeshFluxf()[matI].boundaryField()[patchI]);
                }
                const symmTensorField& fluxPatch = *fluxPatchPtr;

                const labelList& faceCells =
                    subMesh.boundaryMesh()[patchI].faceCells();

                // Assemble the shadow flux field: this is the stress
                // calculated from the other side of the interface (in the
                // subMesh on the other side)
                const symmTensorField& interfaceShadFlux =
                    interfaceShadowFlux()[matI];
                ...
                forAll(Dinterface, faceI)
                {
                    ...
                    if (mesh.isInternalFace(baseFaceID))
                    {
                        ...
                        const vector tractiona = n & fluxPatch[faceI];
                        const vector tractionb = n & interfaceShadFlux[faceI];
...
void Foam::poroSubMeshes::correctBoundarySnGrad
(
...
void Foam::poroSubMeshes::correctBoundarySnGradf
(
    PtrList<volVectorField>& subMeshDList,
...
void Foam::poroSubMeshes::moveSubMeshes()
{
